...
export const CreateChatModal = () => {
  ...
  useEffect(() => {
    if (chatType) {
      form.setValue("type", chatType);
    } else {
      form.setValue("type", ChatType.TEXT);
    }
  }, [chatType, form]);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      // Make the API request and log the response for debugging
      const response = await axios.post(url, values);

      console.log("API Response:", response.data); // Log the full response for debugging

      // Ensure the backend returns the correct new chat ID
      const newChatId = response.data.id; // Ensure this points to the correct new chat's ID

      if (!newChatId) {
        throw new Error("New chat ID not found in the response");
      }
